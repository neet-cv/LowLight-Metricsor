...
_root_path = os.path.join(os.getcwd(), 'Metrics')

def is_image(path: str):
    short_path = ntpath.basename(path)
    kind = os.path.splitext(short_path)[-1]
    if kind not in ['.jpg', '.png', '.bmp', '.jpeg', '.jpe']:
        return False
    return True
...
class Metrics:
    def __init__(self, use_gpu=True if torch.cuda.is_available() else False, name="Metrics"):
